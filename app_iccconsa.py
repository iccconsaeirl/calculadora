...
# FUNCIONES
# ================================================================
def decimal_a_dms(valor, latitud=True, num_decimales=4):
    try:
        valor = float(valor)
    except ValueError:
        return "Error"
...
uploaded_file = st.sidebar.file_uploader("📂 Cargar archivo CSV con columnas E,N,H,Descripcion", type=["csv"])
    ...
    if uploaded_file:
        try:
            ...
            if not required_cols.issubset(df_input.columns):
                ...
                # Intenta convertir las columnas a numérico. Los errores se convierten en NaN.
                ...
                # Revisa si hay valores no numéricos (NaN) en las columnas críticas
             ...
             st.error("Error al leer el archivo CSV. Asegúrate de que el formato sea correcto (delimitador, etc.).")
        except Exception as e:
            st.error(f"Error desconocido al leer el archivo: {e}")
    else:
